...
static void CG_Draw2D( void ) 
{
	...
	// Posto : I get to the principle that this is called at least sv_fps/125 times per second.
	// Which should be, since it's a frame generated by com_maxfps (?) which is also 125 by default
	if (cg_TASActive.integer)
	{
		if (!TAS_Process) {
			TAS_Process = std::make_unique<TAS>();
			//TAS_Process->start();
			Com_Printf("The TAS will start in about 10sec, be prepared !\n");
		}
		TAS_Process->notifyNewFrame(); // Always send the gamestate we want
	}
	else
	{
		if (TAS_Process)
		{
			TAS_Process->stop();
			TAS_Process.reset(); // Release memory
			//TAS_Process.release(); // Release pointer but not the object, can cause memory leak
			Com_Printf("The TAS has been correctly stopped !\n");
		}
	}
